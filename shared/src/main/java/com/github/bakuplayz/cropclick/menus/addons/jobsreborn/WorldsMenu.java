...
import static com.github.bakuplayz.cropclick.language.LanguageAPI.Menu.WORLDS_ITEM_JOBS_TIPS;
import static com.github.bakuplayz.cropclick.language.LanguageAPI.Menu.WORLDS_ITEM_STATUS;

/**
 * A class representing the Worlds menu scoped for JobsReborn.
 *
 * @author BakuPlayz
 * @version 2.2.0
 * @since 2.2.0
...
public final class WorldsMenu extends AbstractWorldsMenu {
    ...
    private static List<String> getItemLore(@NotNull CropClick plugin, @NotNull FarmWorld world) {
        boolean isBanished = world.isBanishedAddon(plugin.getAddonManager(), JobsRebornAddon.NAME);
    ...
    public ItemAction getPaginatedItemAction(@NotNull FarmWorld world, int position) {
        return (item, player) -> stateHandler.toggleWorld(world, JobsRebornAddon.NAME, position);
    }
